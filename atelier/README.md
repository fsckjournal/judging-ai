# ESAR · Un lieu, un SKILL.md, un hook
...
- **Exemple de ce qu’emporte un binôme :** `EXEMPLE_toolkit.zip`. Il contient un SKILL.md illustrative, les domaines fictifs et une comparaison de modèle explicitement en attente. Ce n’est pas un travail étudiant terminé.
...
Le dossier `portable/` de chaque ZIP conserve du texte ordinaire : instructions, demandes sans/avec, ainsi que SKILL.md et, dans le ZIP final, les quatre documents fictifs et les six cas en JSON. Les demandes sont réutilisables dans les trois chats ; aucun manifeste Claude n’est requis pour ce parcours. Les premiers ZIP contiennent déjà les instructions et SKILL.md ; les demandes arrivent après la cellule 12. La comparaison doit rester dans le même modèle et les mêmes conditions, pas changer de fournisseur entre les deux réponses.
...
- Exports portables vérifiés contre les variables réellement exécutées : instructions sans en-tête YAML, deux demandes avec la même question, SKILL.md identique, quatre documents synthétiques, six cas et guide de compatibilité exact.
